...
/**
 * FloorPlanLocationResponse represents a floor plan location in a dining
 * party response.
 *
 * @param floorPlanName
 * @param floorPlanElementIDs
 * @param tableName floorplan element name reserved by order if it's the
 *     only one or combination of 1st and last floorplan element names
 */
@Serializable
data class FloorPlanLocationResponse(
    val floorPlanName: String? = null,
    val floorPlanElementIDs: List<String>? = null,
    /**
     * floorplan element name reserved by order if it's the only one or
     * combination of 1st and last floorplan element names
     */
