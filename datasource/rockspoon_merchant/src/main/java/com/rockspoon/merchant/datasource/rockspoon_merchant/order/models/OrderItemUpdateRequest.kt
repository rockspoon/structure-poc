...
/**
 * OrderItemUpdateRequest represents the order item update request
 * structure
 *
 * @param toGo
 * @param course Course that the item is on. A course is a specific set of
 *     food that are served together during a meal. It must be between 0
 *     and 64. A course 0 means no course.
 * @param numberOfPlates Number of plates that must be brought when serving
 *     this order item
 * @param servingOption
 * @param customerIds ID of the order customers the item belongs to.
 *     Multiple IDs can be passed if it is a shared meal.
 * @param metadata
 * @param routings Only updates routings for manual items
 * @param category Only updates category for manual items
 * @param subcategory Only updates sub category for manual items
...
data class OrderItemUpdateRequest(
    ...
    /**
     * Course that the item is on. A course is a specific set of food that are
     * served together during a meal. It must be between 0 and 64. A course 0
     * means no course.
     */
    val course: String? = null,
    /** Number of plates that must be brought when serving this order item */
    val numberOfPlates: String? = null,
    val servingOption: ServingOptionDto? = null,
    /**
     * ID of the order customers the item belongs to. Multiple IDs can be
     * passed if it is a shared meal.
     */
    val customerIds: List<String>? = null,
    val metadata: OrderItemUpdateMetadataRequest? = null,
    /** Only updates routings for manual items */
    val routings: List<String>? = null,
    /** Only updates category for manual items */
    val category: NullableItemCategory? = null,
    /** Only updates sub category for manual items */
    val subcategory: String? = null,
    val customerAllergens: CustomerAllergensArray? = null
)
