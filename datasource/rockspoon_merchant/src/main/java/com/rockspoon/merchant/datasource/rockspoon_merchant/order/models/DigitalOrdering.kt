...
/**
 * Order settings for floor plan QR code generation
 *
 * @param qrCodeSize
 * @param qrCodeStyle
 * @param layout
 */
@Serializable
data class DigitalOrdering(
    @Serializable(with = BigDecimalSerializer::class)
    val qrCodeSize: BigDecimal? = null,
    val qrCodeStyle: QrCodeStyle? = null,
    val layout: Layout? = null
) {

    /** Values: default,qrcode_only */
    @Serializable
    enum class QrCodeStyle(val value: String) {

        @SerialName("default")
        DEFAULT("default"),

        @SerialName("qrcode-only")
        QRCODE_ONLY("qrcode-only");
