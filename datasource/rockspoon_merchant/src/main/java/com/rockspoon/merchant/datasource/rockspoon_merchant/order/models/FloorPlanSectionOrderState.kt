...
/**
 * @param orderId empty if there is no open order
 * @param shortId empty if there is no open order
 * @param status empty if there is no open order
 * @param missingItems list of orderItemIDs that are unavailable
 * @param showNeedAssistanceIcon if call waiter is true for the table
 * @param allDinnersAreCa if all diners in the table have the flag isCA
 * @param hasCaDiners if at least one diner in the table has the flag isCA
 * @param updatedAt
 * @param notificationCount
 */
@Serializable
data class FloorPlanSectionOrderState(
    /** empty if there is no open order */
    val orderId: String? = null,
    /** empty if there is no open order */
    val shortId: String? = null,
    /** empty if there is no open order */
    val status: FloorPlanSectionElementOrderStatus? = null,
    /** list of orderItemIDs that are unavailable */
    val missingItems: List<String>? = null,
    /** if call waiter is true for the table */
    val showNeedAssistanceIcon: Boolean? = null,
    /** if all diners in the table have the flag isCA */
    val allDinnersAreCa: Boolean? = null,
    /** if at least one diner in the table has the flag isCA */
    val hasCaDiners: Boolean? = null,
    val updatedAt: Instant? = null,
    @Serializable(with = BigDecimalSerializer::class)
    val notificationCount: BigDecimal? = null
)
