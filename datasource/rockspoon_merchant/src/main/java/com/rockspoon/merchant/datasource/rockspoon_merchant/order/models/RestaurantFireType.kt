...
/**
 * Fire type for order items: * `together`: items will be ready all
 * together * `as_ready`: items will be marked as ready as they are
 * finished by the kitchen * `add_on`: add the items to the last card
 * added to the kitchen by that waiter * `rush`: mark the items to be
 * done more fastly * `dont`: these items were already done by the
 * kitchen (e.g. waiter called by the consumer app this array should
 * be empty the kitchen on person). Firing will create the card in the
 * kitchen but it will be marked as delivered. * `timed_fire`: items
 * will be fired after some time * `by_course`: items will be grouped
 * and will be marked as finished according to their `course` Values:
 * together,asReady,addOn,rush,dont,timedFire,byCourse
 */
@Serializable
enum class RestaurantFireType(val value: String) {
