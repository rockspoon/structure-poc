...
/**
 * VoidItemRequest represents an item that will be voided from the order.
 *
 * @param checkId if not specified we search an item in a table check
 * @param reason
 */
@Serializable
data class VoidItemRequest(
    val reason: String,
    /** if not specified we search an item in a table check */
